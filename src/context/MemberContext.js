...
const AddMemberProvider = ({ children }) => {
  ...
  const addMemberHandler = value => {
    setCurrentMember(currentMember => [...currentMember, value]);
  };

  const removeMemberHandler = memberToRemove => {
    const updatedMembers = currentMember.filter(
  ...
  return (
    <AddMemberContext.Provider
      value={{ currentMember, addMemberHandler, removeMemberHandler }}
    >
      {children}
    </AddMemberContext.Provider>
  );
